...
class ManagerBrain:
    """Main reinforcement learning component using Q-learning or DQN."""
    
    def __init__(self, profile, use_dqn: bool = False):
        """
        Initialize the manager brain.
        
        Args:
            profile: Manager profile for personality-based adjustments
            use_dqn: If True, use Deep Q-Network instead of tabular Q-learning
        """
        ...
        if use_dqn:
            # Use DQN for function approximation
            try:
                from ml.dqn_agent import DQNAgent
                self.dqn_agent = DQNAgent(
                    obs_dim=20,  # Observation space dimension
                ...
                print("Warning: DQN not available, falling back to Q-tables")
                self.use_dqn = False
        
        # Tabular Q-tables (legacy, used if use_dqn=False)
    ...
    def load_model(self, path: str):
        ...
        if self.dqn_agent:
            ...
            print(f"ManagerBrain: Loaded DQN model from {path}")
        else:
            print("Warning: DQN agent not initialized, cannot load model")
    
    def select_action(self, state: Tuple, possible_actions: List[Any],
                     action_type: str = "match", raw_state: Optional[Dict] = None) -> Any:
        """Select action using either DQN or epsilon-greedy Q-learning."""
        
        # Use DQN if enabled and action is one of the 5 standard types
        if self.use_dqn and self.dqn_agent and raw_state and len(possible_actions) == 5:
            # Convert raw_state to 20-dim observation vector
            obs = self._get_observation_vector(raw_state)
            # Select action index (0-4)
            action_idx = self.dqn_agent.select_action(obs, training=False)
    ...
    def _get_observation_vector(self, raw_state: Dict) -> np.ndarray:
        """
        Convert simulation state into the 20-dim vector used by DQN.
        This mirrors the logic in FootyEnv._get_observation().
        """
        obs = np.zeros(20, dtype=np.float32)
        
        # Self Context (0-4)
        obs[0] = min(1.0, raw_state.get("budget", 0) / 200_000_000)
        obs[1] = raw_state.get("squad_size", 0) / 30.0
        obs[2] = raw_state.get("avg_age", 25) / 40.0
        obs[3] = raw_state.get("avg_attack", 50) / 100.0
        obs[4] = raw_state.get("avg_defense", 50) / 100.0
        
        # League Context (5-8)
        obs[5] = raw_state.get("position", 10) / 20.0
        obs[6] = raw_state.get("points", 0) / 100.0
        obs[7] = np.clip(raw_state.get("goal_diff", 0) / 100.0, -1, 1) * 0.5 + 0.5
        obs[8] = raw_state.get("weeks_remaining", 0) / 38.0
        
        # Market Context (9-10)
        obs[9] = 1.0 if raw_state.get("transfer_window_open") else 0.0
        obs[10] = raw_state.get("market_inflation", 1.0) / 2.0
        
        # Squad Quality (11-15)
        obs[11] = raw_state.get("avg_potential", 75) / 100.0
        obs[12] = raw_state.get("avg_form", 0.5)
        obs[13] = raw_state.get("injured_pct", 0)
        obs[14] = raw_state.get("youth_ratio", 0.2)
        obs[15] = raw_state.get("star_ratio", 0.1)
        
        # Performance Trends (16-19)
        obs[16] = raw_state.get("recent_win_rate", 0)
        obs[17] = raw_state.get("recent_draw_rate", 0)
        obs[18] = raw_state.get("goals_per_game", 1.0) / 3.0
        obs[19] = raw_state.get("conceded_per_game", 1.0) / 3.0

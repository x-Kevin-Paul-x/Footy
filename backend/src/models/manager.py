...
class Manager:
    def __init__(self, name=None, experience_level=None, profile=None,
                 ...
                 use_dqn: bool = False):
        """Initialize a manager with Q-learning or DQN capabilities."""
        ...
        self.brain = ManagerBrain(self.profile, use_dqn=use_dqn)
    ...
    def load_from_database(cls, manager_id):
        ...
        if isinstance(data, tuple):
            ...
            manager.formation = data[7]
            manager.matches_played = data[8]
            manager.wins = data[9]
            manager.draws = data[10]
            manager.losses = data[11]
            manager.total_rewards = data[12]
        else:
            manager = cls(
                name=data["name"],
    ...
    def make_transfer_decision(self, transfer_market):
        """Make transfer decisions using DQN or Q-learning. 
           Selling is heuristics-based (to provide market liquidity). 
           Buying is DQN-based (strategic).
        """
        raw_state = self.get_state()
        state = self.brain.encode_state(raw_state)
        
        # Get all possible actions (these include 'sell' and 'buy')
        possible_actions = self._get_possible_transfer_actions(transfer_market)
        
        # Initialize result list
        final_actions = []
        
        # --- 1. ALWAYS PROCESS SELLING (Generates Market Liquidity) ---
        # If we have "sell" actions generated by heuristics (surplus players), take them.
        # We limit to 1 sell per turn to avoid mass dumping, unless in financial crisis.
        sell_actions = [a for a in possible_actions if a[0] == "sell"]
        if sell_actions:
            # If in debt, sell aggressively. Otherwise, sell one at a time.
            if self.team.budget < 0:
                final_actions.extend([("list", a[1], a[2]) for a in sell_actions])
            else:
                # Randomly pick one to sell
                if random.random() < 0.3: # 30% chance to list a surplus player per day
        ...
        if self.brain.use_dqn:
            ...
            if dqn_action == 0:  # Do Nothing
                pass
            elif dqn_action == 1:  # Scout
                if hasattr(self, '_debug') and self._debug:
                    print(f"{self.name} decided to scout (DQN Action 1)")
            else:
                # Mapping DQN actions (2,3,4) to granular "buy" actions
                target_range = {
                    2: (0, 70),   # Cheap/Youth
                    3: (70, 80),  # Value/Prime
                    4: (80, 100)  # Star
                ...
                if eligible_buys:
                    # Pick best eligible buy within the DQN's chosen category
                    best_option = max(eligible_buys, key=lambda x: x[1].player.get_overall_rating())
                    q_action = best_option[0]
                    # Convert q_action ('buy', id, price) to market action
                    dqn_result_actions = self._convert_q_action_to_transfer_actions(q_action, transfer_market)

        else:
            # Heuristic buying for non-AI managers
            # 1. Identify needs
            squad_needs = self.team.get_squad_needs()
            needed_positions = {need["position"] for need in squad_needs.get("needs", [])}
            
            # 2. Check budget (reserve 20% for wages/emergency)
            available_budget = self.team.transfer_budget * 0.8
            
            # 3. Scan market for needed positions
            market_listings = transfer_market.get_available_players(max_price=available_budget)
            
            for listing in market_listings:
                if len(dqn_result_actions) > 0: 
                    break # Limit to 1 buy attempt per turn to be safe
                
                player = listing.player
                
                if player.position in needed_positions:
                    # Check if player improves the squad
                    pos_analysis = squad_needs["position_analysis"].get(player.position, {})
                    current_quality = pos_analysis.get("starter_quality", 0)
                    
                    if player.get_overall_rating() > current_quality * 1.02: # 2% better
                         # Make offer
                         offer_price = listing.asking_price * random.uniform(0.95, 1.1)
                         if self.team.can_afford_transfer(offer_price, player.wage):
                             dqn_result_actions.append(("buy", listing.listing_id, offer_price))


        # Combine actions
        final_actions.extend(dqn_result_actions)
        
        # Add a "none" action if empty to satisfy return type expectations if needed, 
        # but the simulation loop iterates over the list, so empty is fine.
        
        # Debug output if enabled
        if hasattr(self, '_debug') and self._debug and final_actions:
             print(f"\n{self.name} Actions: {len(final_actions)}")
    ...
    def _get_possible_transfer_actions(self, transfer_market):
        ...
        if len(self.team.players) > 18:
            for player in self.team.players:
                ...
                pos_info = squad_needs.get("position_analysis", {}).get(position_group, {})
                current, ideal = pos_info.get("current_players", 0), pos_info.get("ideal_total", 0)
                average_rating = self.team.get_squad_strength()

                # CRITERIA 1: Age cleanup
                is_old = player.age > 30 and player.get_overall_rating() < average_rating
                
                # CRITERIA 2: Surplus to requirements
                is_surplus = ideal > 0 and current > ideal and player.squad_role not in ["STARTER"]
                
                # CRITERIA 3: Contract Expiry (1 year left and not a key player)
                is_expiring = player.contract_length <= 1 and player.squad_role != "STARTER"

                # CRITERIA 4: Dead weight (significantly below team average)
                is_deadweight = player.get_overall_rating() < (average_rating - 5)

                should_sell = (is_old or is_surplus or is_expiring or is_deadweight) and not player.recently_transferred

                if should_sell:
                    # Higher probability of selling if deadweight
                    prob = 0.5 if is_deadweight or is_surplus else 0.3
                    if random.random() < prob:
                         actions.append(("sell", player, market_value * random.uniform(0.9, 1.1)))
                         
                # Loan logic for youngsters
                if player.age < 23 and player.squad_role in ["RESERVE", "YOUTH"] and player.potential > 75:
                     if random.random() < 0.3:
                         actions.append(("loan_out", player))
    ...
    def _convert_q_action_to_transfer_actions(self, q_action, transfer_market):
        ...
        """
        if not q_action or q_action[0] in ["none", "do_nothing"]:
            return []

        action_type = q_action[0]
        
        if action_type == "sell":
            # ('sell', player, price)
            target, price = q_action[1], q_action[2]
            return [("list", target, price)]

        elif action_type == "buy":
            # ('buy', listing, offer) or ('buy', listing_id, offer)
            # Check if we have the listing object or id
            target = q_action[1] # listing or id
            price = q_action[2]

...
class FootballPlayer:
    ...
    def __init__(self, name, age, position, potential=70, wage=1000):
        ...
        self.yellow_cards_accumulator = 0
    ...
    def save_to_database(self, team_id=None):
        ...
        if self.player_id is None:
            ...
            self.player_id = create_player(
                ...
                team_id=db_team_id if db_team_id is not None else -1,
    ...
    def get_suspension_games(self):
        """Get remaining suspension games"""
        return self.suspended_matches

    def serve_suspension(self, games=1):
        """Serve suspended games"""
        self.suspended_matches = max(0, self.suspended_matches - games)

    def add_card(self, card_type="yellow"):
        """Add a yellow or red card and handle suspension accumulation"""
        if card_type == "yellow":
            self.stats["yellow_cards"] += 1
            self.yellow_cards_accumulator += 1
            if self.yellow_cards_accumulator >= 5:
                self.suspended_matches += 1
                self.yellow_cards_accumulator = 0
        elif card_type == "red":
            self.stats["red_cards"] += 1
            self.suspended_matches += 3
    ...
    def train_player(self, intensity, focus_area=None, training_days=1, coach_bonus=0):
        ...
        # Complete results
        results["final_attributes"] = {
            attr_type: {sub_attr: value for sub_attr, value in sub_attributes.items()}
            for attr_type, sub_attributes in self.attributes.items()
        }

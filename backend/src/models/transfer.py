...
class TransferMarket:
    def __init__(self, log_path=None):
        ...
        if log_path is None:
            log_path = os.path.join(self.log_dir, f'season_{self.season_year}_transfers.txt')

        self.current_log = None
        if log_path:
            # If log_path is just a filename, put it in log_dir
            if not os.path.isabs(log_path) and os.sep not in log_path:
                log_path = os.path.join(self.log_dir, log_path)
    ...
    def _calculate_heuristic_player_value(self, player) -> float:
        """Heuristic fallback valuation used when no model-backed estimate is available."""
    ...
    def _estimate_model_player_value(self, player, buying_team=None, reference_teams=None) -> Optional[float]:
        """Estimate player value using the supervised market model when enough context is available."""
        valuator = get_market_valuator()
        if not valuator or not buying_team:
            return None

        league_avg_budget = None
        if reference_teams:
            budgets = [team.budget for team in reference_teams if team is not None and hasattr(team, "budget")]
            if budgets:
                league_avg_budget = sum(budgets) / len(budgets)

        if league_avg_budget is None:
            league_avg_budget = buying_team.budget

        try:
            return valuator.predict_from_player(player, buying_team, league_avg_budget)
        except Exception:
            return None

    def calculate_player_value(self, player, buying_team=None, reference_teams=None) -> float:
        """Enhanced player valuation with optional supervised-model support."""
        heuristic_value = self._calculate_heuristic_player_value(player)
        model_value = self._estimate_model_player_value(
            player,
        ...
        blended_value = (heuristic_value * 0.5) + (model_value * 0.5)
        return max(500000, round(blended_value))
    ...
    def make_transfer_offer(self, buying_team, listing, offer_amount):
        ...
        reference_teams = [buying_team, listing.selling_team]
        market_value = self.calculate_player_value(
            player,
            buying_team=buying_team,
            reference_teams=reference_teams,
        )
        min_acceptable = max(listing.asking_price * 0.75, market_value * 0.85)

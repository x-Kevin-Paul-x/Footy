...
# Relative to backend/src/database/db_setup.py
# New location: backend/data/football_sim.db
...
def create_tables(db_file=DB_FILE):
    ...
    # SeasonReport Table
    cursor.execute("""
        CREATE TABLE IF NOT EXISTS SeasonReport (
    ...
    # TransferReport Table
    cursor.execute("""
        CREATE TABLE IF NOT EXISTS TransferReport (
            report_id INTEGER PRIMARY KEY AUTOINCREMENT,
            season_year INTEGER NOT NULL,
            report_data TEXT NOT NULL,
            created_at TEXT NOT NULL
